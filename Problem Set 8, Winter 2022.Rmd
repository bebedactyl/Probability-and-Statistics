...
tvt2.rep <- rep(0:1,c(round(n*.3),n-2*round(n*.15)))
tvt2.rep
table(tvt2.rep)
...
tv.split <- sample(rep(0:1,c(round(n*.3),n-2*round(n*.15))))
...
a<-model.matrix(medv~.,data=dat.train)
a <- a[,-1]
b<-dat.train$medv
...
cvfit.house.ridge <- cv.glmnet(x=a, y=b,alpha=0)# Complete this line
...
Your answer here: accelerated failure time model with an exponential based distribution
 
C) What does the output of the model fitted using the survreg() function suggest about the treatment groups (A, B, and C)?

Your answer here:B and C have negative coefficients meaning they have a faster dying off rate than A.
...
Your answer here: Cummulative distribution curves. The expected outcome.
