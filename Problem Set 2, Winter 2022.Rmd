...
doughnuts.factorial <- doughnuts.factorial %>% unite("Cell", fat_type:flour_type, remove = FALSE)
...
ggqqplot(doughnuts.factorial,x="sim_tot_fat",facet.by = "Cell")
...
ggplot(data=doughnuts.factorial, aes(y=sim_tot_fat))+geom_boxplot()+facet_grid(rows=vars(flour_type),cols=vars(fat_type))
...
leveneTest(sim_tot_fat ~ fat_type*flour_type, data = doughnuts.factorial)
