...
model.interaction <- lm(sim_tot_fat ~ fat_type_factor*flour_type_factor, data = doughnuts.factorial) # Complete this line
...
fmla <- as.formula(str_c("logTotHg ~ ", str_c(names(fish.auto)[1:(ncol(fish.auto)-1)], collapse= "+")))

forward.model<-step(lm(logTotHg~1,data=fish.auto),scope = fmla, direction="forward", trace = 1)
...
fmla.max = as.formula("logTotHg ~ fisherman_factor + age + restime + height + weight + fishmlwk + fishpart_factor")

fmla.null = as.formula("logTotHg ~ 1")

backward.model<-step(lm(fmla,data=fish.auto),scope=fmla.null, direction="backward", trace = 1)
...
x<-model.matrix(fmla.max, data=fish.auto)
...
best.subset.aic<-which(AICs==min(AICs))[1]
best.subset.aic
