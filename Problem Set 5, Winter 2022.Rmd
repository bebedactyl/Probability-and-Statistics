...
prob.from.odds <- function(b){b/(1+b)}
...
x <- c(5, 10, 20) 
y <- c(prob.from.odds(5), prob.from.odds(10), prob.from.odds(20))
...
Your answer here: As the odds increase, the probability becomes closer to 1.1
...
pew.complete<-subset(pew, IDEO!="-1"& LIFE!="-1")
...
Your answer here: 1570 (better today) + 492 (about same) = 2062 
...
pew.complete$worse <- pew.complete$LIFE
pew.complete$worse[pew.complete$worse=="1"]<-"0"
pew.complete$worse[pew.complete$worse=="3"]<-"0"
pew.complete$worse[pew.complete$worse=="2"]<-"1"
...
Model.1 <- glm(worse~income + gender, data=pew.complete, family="binomial")# Complete this line
...
Model.2 <- glm(worse~income + gender + eth + edu, data=pew.complete, family="binomial")# Complete this line
...
Model.3 <- glm(worse~income + gender + eth + edu + ideo, data=pew.complete, family="binomial") # Complete this line
...
m1.vs.m2 <- anova(Model.1, Model.2)
...
m2.vs.m3 <-anova(Model.2, Model.3)
